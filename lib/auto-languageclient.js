...
import {ConsoleLogger, FileLogger, CompositeLogger, NullLogger, type Logger} from './logger';
...
export default class AutoLanguageClient {
  ...
  getLogger(): Logger {
    const logConfig = atom.config.get('core.debugLSP');
    if (logConfig == null) {
    ...
    for (const destination of logConfig.split(';')) {
      const parts = destination.split('=');
      switch (parts[0]) {
        case 'file': {
          const fileLogger = new FileLogger();
          fileLogger.open(parts[1]);
          logger.addLogger(fileLogger);
          break;
        }
        case 'console': {
          logger.addLogger(new ConsoleLogger(this.name));
          break;
        }
      }
    }
